export const Products = [
    {
        name: "Monitoring Hub",
        description: "Stay ahead with real-time monitoring and optimization of AI/ML model performance",
        href: "/Products/Monitoring",
        icon: "/pr1.png",
      },
      {
        name: "Observability Hub",
        description: "Unlock the full potential of your AI/ML models with comprehensive observability and actionable insights",
        href: "/Products/Observability",
        icon: "/pr2.png",
      },
      {
        name: "Governance Hub",
        description: "Effortlessly govern your AI/ML models enterprise-wide for compliance and risk management",
        href: "/Products/Governance",
        icon: "/pr3.png",
      },
];
export const Solutions = [
    {
        name: "Finance and Banking ",
        description: "Monitor AI/ML algorithms for regulatory compliance, ensuring transparency and accountability in financial services.",
        href: "#",
        icon: "/so1.png",
      },
      {
        name: "Insurance Monitoring ",
        description: "Ensure transparency and compliance in insurance AI and ML models through rigorous governance, observability, and monitoring of data.",
        href: "#",
        icon: "/so5.png",
      },
      {
        name: "Real Estate",
        description: "Monitor AI algorithms to uphold fair housing laws and maintain transparency in property pricing, establishing governance in real estate AI.",
        href: "#",
        icon: "/so2.png",
      },
      {
        name: "Retail and E-commerce",
        description: "Refine decision-making in retail operations by optimizing models for accuracy and reliability.",
        href: "#",
        icon: "/so6.png",
      },
      {
        name: "Healthcare",
        description: "Maintain transparency and regulatory compliance in healthcare AI models with comprehensive observability and monitoring.",
        href: "#",
        icon: "/so3.png",
      },
      {
        name: "Autonoumus Vehicle",
        description: "Enhance the interpretation and interaction of real-world data to facilitate AI-driven decision-making.",
        href: "#",
        icon: "/so7.png",
      },
      {
        name: "Social Media ",
        description: "Maintain platform integrity and regulatory adherence in social media AI through the implementation of governance protocols and ongoing observability.",
        href: "#",
        icon: "/so4.png",
      },
      {
        name: "Education",
        description: "Secure transparency and fairness in education AI/ML models with stringent governance and perpetual observability.",
        href: "#",
        icon: "/so8.png",
      },
];
export const Pricing = [
    {
        name: "Tabular",
        description: "Monitor structured, tabular data to ensure accuracy, bias and fairness.",
        href: "/Pricing",
        icon: "/pri1.png",
      },
      {
        name: "Text",
        description: "Ensure ethical language processing in natural language based data sources.",
        href: "#",
        icon: "/pri3.png",
      },
      {
        name: "Image",
        description: "Detect biases and promote inclusivity in images and medical scans through monitoring and observability.",
        href: "#",
        icon: "/pri2.png",
      },
      {
        name: "LLMs",
        description: "Monitor ethical language use, hallucination, context accuracy  and misinformation detection.",
        href: "#",
        icon: "/pri4.png",
      },
];
export const Docs = [
    {
        name: "Resource Library",
        description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua.",
        href: "#",
        icon: "/doc1.png",
      },
      {
        name: "Documentation",
        description: "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. ",
        href: "#",
        icon: "/doc2.png",
      },
];
export const Recources = [
    {
        name: "Blog",
        description: "Stay informed with the latest AI trends, news, and in-depth analysis from industry leaders. ",
        href: "/recourses/Blog",
        icon: "/r1.png",
      },
      {
        name: "Support",
        description: "Get expert assistance and find answers to your questions with our comprehensive support resources",
        href: "#",
        icon: "/r3.png",
      },
      {
        name: "FAQs",
        description: "Explore our comprehensive FAQs to resolve your queries and learn more about our services.",
        href: "/recourses/FAQ",
        icon: "/r2.png",
      },
      {
        name: "Integration ",
        description: "Learn about our flexible integration options to ensure smooth AI implementation.",
        href: "#",
        icon: "/r4.png",
      },
];
export const Company = [
    {
        name: "Career",
        description: "Explore rewarding career opportunities and unleash your potential with us.",
        href: "#",
        icon: "/co1.png",
      },
      {
        name: "About Us",
        description: "Discover who we are, what we do, and why we're passionate ",
        href: "/About",
        icon: "/co3.png",
      },
      {
        name: "Contact Us",
        description: "Reach out to us for inquiries, collaborations, or to learn more about Us.",
        href: "#",
        icon: "/co2.png",
      },
      
];