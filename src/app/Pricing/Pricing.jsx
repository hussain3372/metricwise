...
const Pricing = () => {
  ...
  const handleExpand = (index) => {
    setExpandedIndexes((prev) => {
      const currentCount = prev[index] || 11; // Get the current expanded count, default to 4
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-10 px-5 md:px-20 py-10 md:py-0 absolute z-10 top-[410px]">
          {Price && Price.length > 0 ? (
            Price.map((item, index) => {
              ...
              return (
                <div
                  className="bg-[#FFF] shadow-[0_3px_10px_rgb(0,0,0,0.2)] p-6 rounded-[20px] mb-10 flex flex-col justify-between"
                  key={`card-${index}`}
                >
                  <div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 justify-center items-center gap-3 mt-6">
                      ...
                      <button
                        className="text-white font-20 font-medium leading-6 px- py-3.5 bg-[#2E2E2E]  rounded-lg"
