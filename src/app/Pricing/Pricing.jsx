          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-10 px-5 md:px-10 py-10 md:py-0">
            {Price && Price.length > 0 ? (
              Price.map((item, index) => (
                <Tilt key={`tilt-${index}`} scale={scale} transitionSpeed={2500}>
                  <div
                    className="bg-[#FFF] shadow-[0_3px_10px_rgb(0,0,0,0.2)] p-6 rounded-[20px] mb-10"
                    key={`card-${index}`}
                  >
                    <h1 className="font-36 fontbold leading-40 textcolor">{item.title}</h1>
                    <ul className="mt-3">
                    ...
                    <div className="mt-20 font-20 leading-6 fontmedium">
                      {item.months}
                      <span className="font-16 fontmedium opacity-60 leading-5">{item.span}</span>
                    </div>
